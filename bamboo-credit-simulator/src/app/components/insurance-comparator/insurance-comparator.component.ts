...
@Component({
  ...
  template: `
    <div class="insurance-comparator-container">
                  ...
                  <div *ngFor="let offer of comparisonResult.offers.slice(1)" class="quote-card">
                    <div class="quote-header">
                      <div class="insurer-info">
                        <h4>{{ offer.insurerName }}</h4>
                        <span class="product-name">{{ offer.productName }}</span>
                      </div>
                      <div *ngIf="offer.customerRating" class="rating">
                        <span class="rating-value">{{ offer.customerRating }}/5</span>
                        <div class="stars">
                          <span *ngFor="let star of getStarsArray(offer.customerRating)" class="star">★</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div class="quote-actions">
                      ...
                      <button (click)="selectOffer(offer)" class="btn-primary">
                        Choisir cette offre
                      </button>
                    </div>
                  </div>
      ...
      <!-- Loading Overlay -->
      <div *ngIf="isLoadingProducts || isLoadingQuote" class="loading-overlay">
        <div class="loading-content">
          <div class="spinner"></div>
          <p>{{ isLoadingProducts ? 'Chargement des produits...' : 'Calcul de votre devis...' }}</p>
        </div>
      </div>
    </div>
  `,
